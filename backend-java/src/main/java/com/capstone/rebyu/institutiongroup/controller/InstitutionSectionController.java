...
import com.capstone.rebyu.institutiongroup.repository.InstitutionSectionRepository;
 ...
 * department head of this group -- via {@link InstitutionGroupService#getAccessibleById}.
 */
@RestController
@RequestMapping("/api/institution-groups/{groupId}/sections")
...
public class InstitutionSectionController {
    ...
    public List<SectionDto> list(@AuthenticationPrincipal Jwt jwt, @PathVariable Long groupId) {
        requireGroupAccess(jwt, groupId);
        return sections
                .findByInstitutionGroup_InstitutionGroupIdAndStatusOrderByCreatedAtAsc(groupId, InstitutionSection.Status.active)
    ...
    public SectionDto create(
        ...
        if (sections.existsByInstitutionGroup_InstitutionGroupIdAndSectionNameIgnoreCaseAndStatus(
                groupId, name, InstitutionSection.Status.active)) {
    ...
    public SectionDto update(
            ...
            @Valid @RequestBody SectionRequest request) {
        requireGroupAccess(jwt, groupId);
        InstitutionSection section = find(groupId, sectionId);
        String name = request.sectionName().trim();
        if (!name.equalsIgnoreCase(section.getSectionName())
                && sections.existsByInstitutionGroup_InstitutionGroupIdAndSectionNameIgnoreCaseAndStatus(
                        groupId, name, InstitutionSection.Status.active)) {
            throw new IllegalArgumentException("A section named \"" + name + "\" already exists in this department.");
        }
    ...
    @Transactional
    public void archive(@AuthenticationPrincipal Jwt jwt, @PathVariable Long groupId, @PathVariable Long sectionId) {
        requireGroupAccess(jwt, groupId);
        InstitutionSection section = find(groupId, sectionId);
        entityManager.createQuery(
                        "update InstitutionGroupAssignee a set a.section = null where a.section.sectionId = :id")
    ...
    public Map<String, Object> move(
        ...
        requireGroupAccess(jwt, groupId);
        InstitutionGroupAssignee assignee = assignees.findById(assigneeId)
                .filter(a -> Objects.equals(a.getInstitutionGroup().getInstitutionGroupId(), groupId))
                .orElseThrow(() -> new EntityNotFoundException("That learner is not in this department."));
        InstitutionSection target = request.sectionId() == null ? null : find(groupId, request.sectionId());
        assignee.setSection(target);
        assignees.save(assignee);
        Map<String, Object> out = new java.util.HashMap<>();
        out.put("institutionGroupAssigneeId", assigneeId);
        out.put("sectionId", target == null ? null : target.getSectionId());
        out.put("sectionName", target == null ? null : target.getSectionName());
        return out;
    }

    private InstitutionSection find(Long groupId, Long sectionId) {
        return sections.findBySectionIdAndInstitutionGroup_InstitutionGroupId(sectionId, groupId)
    ...
    private SectionDto toDto(InstitutionSection s) {
        long learners = entityManager.createQuery(
                        "select count(a) from InstitutionGroupAssignee a where a.section.sectionId = :id and a.status = :st", Long.class)
                .setParameter("id", s.getSectionId())
                .setParameter("st", InstitutionGroupAssignee.Status.active)
                .getSingleResult();
        long pending = entityManager.createQuery(
                ...
                .getSingleResult();
        return new SectionDto(s.getSectionId(), s.getInstitutionGroup().getInstitutionGroupId(), s.getSectionName(),
